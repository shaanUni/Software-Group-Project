...
def _filtered_users_queryset(User, params):
    ...
    users = User.objects.select_related("team")
...
def admin_user_management(request):
    ...
    if request.method == "POST":
        ...
        if action in {"bulk_deactivate", "bulk_assign", "export_csv"}:
            ...
            writer.writerow(["Username", "Name", "Email", "Staff", "Superuser", "Active", "Teams"])
            ...
            for account in users_for_export:
                ...
                team_names = ", ".join((team.team_name or f"Team {team.team_id}") for team in assigned)
                writer.writerow(
                    [
                        account.username,
                        account.get_full_name() or account.username,
                        account.email,
                        "yes" if account.is_staff else "no",
                        "yes" if account.is_superuser else "no",
        ...
        if action == "delete":
            if not request.user.is_superuser:
                ...
                # Explicitly clear team assignment before deletion.
                if has_additional_teams:
        ...
        if action == "update":
            ...
            if selected_teams:
                target_user.team = selected_teams[0]
            else:
                target_user.team = None
    ...
    for account in users:
        ...
        if account.team and all(team.pk != account.team.pk for team in assigned):
            assigned.insert(0, account.team)
        user_teams[account.pk] = assigned
